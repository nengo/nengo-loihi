...
def get_gain_bias(ens, rng=np.random, intercept_limit=1.0):
    ...
    if ens.gain is not None and ens.bias is not None:
        ...
        if gain is not None and (not np.all(np.isfinite(gain)) or np.any(gain <= 0.0)):
            raise BuildError(
                "%s: The specified intercepts lead to neurons with negative or "
                ...
                "by reducing the maximum intercept value to below 1." % ens
            )
...
def build_ensemble(model, ens):
    ...
    if np.any(np.isnan(encoders)):
        raise BuildError(
            "%s: NaNs detected in encoders. This usually means that you have "
            "zero-length encoders; when normalized, these result in NaNs. Ensure all "
            "encoders have non-zero length, or set `normalize_encoders=False`." % ens
        )
...
def build_neurons(model, neurontype, neurons, block):
    ...
    raise BuildError(
        "%s: The neuron type %r cannot be simulated on Loihi. Please either "
        "switch to a supported neuron type like LIF or SpikingRectifiedLinear, "
        "or explicitly mark ensembles using this neuron type as off-chip with\n"
        "  net.config[ensembles].on_chip = False" % (neurons, type(neurontype).__name__)
    )
