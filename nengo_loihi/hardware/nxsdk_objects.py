...
class LoihiSpikeInput:
    ...
    def spikes_to_loihi(self, t, input_idxs):
        ...
        Returns
        -------
        spikes : list of ndarray(dtype=spike_dtype)
            Spikes targeting physical locations on the chip.
        """
        if len(input_idxs) == 0:
            return np.array([], dtype=self.spike_dtype)

        spikes = [axon for i in input_idxs for axon in self.axon_map[i]]
        for spike in spikes:
            spike["t"] = t  # Note: this actually changes the values in axon_map
