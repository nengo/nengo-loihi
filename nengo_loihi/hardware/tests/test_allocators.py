...
def test_partition_comms(Simulator, rng):
    ...
    with nengo_loihi.Simulator(net, target="sim") as sim:
        ...
        board_norates = PartitionComms()(model, n_chips=n_chips)
        board_rates = PartitionComms(ensemble_rates=ens_rates)(model, n_chips=n_chips)
    ...
    print(
        "No rates: %d axons, %d spikes"
        % (
            norates_axons["interchip"],
            norates_spikes["interchip"],
        )
    )
    print(
        "Rates: %d axons, %d spikes"
        % (
            rates_axons["interchip"],
            rates_spikes["interchip"],
        )
    )
