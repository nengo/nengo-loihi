...
class CxGroup(object):
    ...
    def __init__(self, n, label=None, location='core'):
        ...
        # self.vmin = np.zeros(n, dtype=np.float32)  # must be same for whole core
        # self.vmax = np.zeros(n, dtype=np.float32)  # must be same for whole core
        self.vmin = 0
        self.vmax = np.inf
    ...
    def add_synapses(self, synapses, name=None):
        ...
        assert sum(s.weights.size for s in self.synapses) < 4*(MAX_MEM_LEN - len(self.synapses))
    ...
    def discretize(self):
        ...
        u_scale = self.decayU.copy() if self.scaleU else np.ones_like(self.decayU)
        v_scale = self.decayV.copy() if self.scaleV else np.ones_like(self.decayV)
        ...
        if len(w_maxs) > 0:
            ...
            # for wgtExp in range(7, -8, -1):
                wgtExp2 = wgtExpBase + wgtExp
                x_scale = s_scale * w_scale * 2**wgtExp2
                b_scale = x_scale * v_scale
                vth = self.vth * x_scale
                vth2 = np.round(vth / 2**6)
                bias = np.round(self.bias * b_scale)
                if np.all(vth2 <= VTH_MAX) and np.all(bias <= BIAS_MAX):
                    vth = vth2 * 2**6
                    break
            else:
...
class CxSynapses(object):
    ...
    def format(self, **kwargs):
        ...
        for key, value in kwargs.items():
            setattr(self.synapse_fmt, key, value)
...
class CxSimulator(object):
    ...
    def build(self, model):
        ...
        def decay_int(x, u, d, s, a=12, b=0):
            ...
            x = np.sign(x) * np.right_shift(np.abs(x) * r, a)  # round towards zero
            return x + u  # no scaling on u

        if group_dtype == np.int32:
            self.decayU_fn = lambda x, u: decay_int(x, u, d=self.decayU, s=self.scaleU)
            self.decayV_fn = lambda x, u: decay_int(x, u, d=self.decayV, s=self.scaleV)
        ...
        self.vmin = np.hstack([group.vmin*ones(group.n) for group in self.groups])
        self.vmax = np.hstack([group.vmax*ones(group.n) for group in self.groups])
        print(self.vth)
